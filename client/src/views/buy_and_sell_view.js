...
BuyAndSellView.prototype.bindEvents = function () {
  ...
  //restore to following channel:
  // PubSub.subscribe('ShareItemView:buy-botton-clicked', (event) => {
  PubSub.subscribe('NavView:market-button-clicked', (event) => {
    const shareObject = event.detail;
    // put the shareObject into following function:
    this.renderBuyAShare(this.sampleObject);
  })
};
...
BuyAndSellView.prototype.renderBuyAShare = function (share) {
  ...
  buy_form.addEventListener('submit', function (event) {
    ...
    // PubSub.publish('BuyAndSellView:BuyShare',event.detail)
  });
